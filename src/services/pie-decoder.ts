...
// =========================================================================
// PIE Message interface
// =========================================================================

export interface PIEMessage {
  id: number
  direction: 'R2T'
  command: string
  roundId: number
...
interface PIEDecoderOptions {
  sampleRate: number
  centerFreqMHz: number
  onMessage: (msg: PIEMessage) => void
}
...
interface PIEDecoderStats {
  preamblesSeen: number
  commandsDecoded: number
  crcErrors: number
}
...
function crc5(bits: number[]): number {
  let crc = CRC5_PRESET;
...
function crc16(bits: number[]): number {
...
function bitsToInt(bits: number[], start: number, length: number): number {
  let val = 0;
...
function bitsToHex(bits: number[], start: number, length: number): string {
 ...
 */
function extractEBV(bits: number[], start: number): EBVResult {
  let value = 0;
  let pos = start;
  let more = true;
...
function parseGen2Command(bits: number[]): ParsedCommand | null {
  ...
  switch (prefix8) {
    case 0b11000000:
      return parseReqRN(bits);
    case 0b11000001:
      return parseRead(bits);
    case 0b11000010:
      return parseWrite(bits);
    case 0b11000011:
      return parseKill(bits);
    case 0b11000100:
      return parseLock(bits);
    case 0b11000101:
      return parseAccess(bits);
    case 0b11000110:
      return parseBlockWrite(bits);
    case 0b11000111:
      return parseBlockErase(bits);
    // Gen2v2 security
    case 0b11010000:
      return parseAuthenticate(bits);
    case 0b11010001:
      return parseAuthComm(bits);
    case 0b11010010:
      return parseSecureComm(bits);
    case 0b11010100:
      return parseTagPrivilege(bits);
    // Gen2X (Impinj)
    case 0b11100000:
      return parseQueryX(bits);
    case 0b11100001:
      return parseQueryY(bits);
    default:
      break;
  }
  ...
  return {
    command: 'Unknown',
    params: {
      prefix: bitsToHex(bits, 0, Math.min(bits.length, 16)),
...
function parseQueryRep(bits: number[]): ParsedCommand {
  // 00 + 2-bit session = 4 bits total
  if (bits.length < 4)
    return { command: 'QueryRep', params: {}, crcValid: null };
  const session = SESSION_MAP[bitsToInt(bits, 2, 2)];
  return { command: 'QueryRep', params: { session }, crcValid: null };
}

function parseACK(bits: number[]): ParsedCommand {
...
function parseQuery(bits: number[]): ParsedCommand {
  // 1000 + DR(1) + M(2) + TRext(1) + Sel(2) + Session(2) + Target(1) + Q(4) + CRC5(5) = 22 bits
  if (bits.length < 22)
    return { command: 'Query', params: { partial: true }, crcValid: null };
...
function parseQueryAdjust(bits: number[]): ParsedCommand {
  // 1001 + session(2) + UpDn(3) = 9 bits
  if (bits.length < 9)
    return { command: 'QueryAdjust', params: {}, crcValid: null };
...
function parseSelect(bits: number[]): ParsedCommand {
  // 1010 + Target(3) + Action(3) + MemBank(2) + Pointer(EBV) + Length(8) + Mask(var) + Truncate(1) + CRC16(16)
  if (bits.length < 26)
    return { command: 'Select', params: { partial: true }, crcValid: null };
  ...
  if (pos + 8 > bits.length)
    return {
      command: 'Select',
      params: {
        target: target <= 3 ? SESSION_MAP[target] : 'SL',
        action,
        memBank,
        pointer,
      },
...
function parseNAK(_bits: number[]): ParsedCommand {
  // 10110000 = 8 bits (padded or just 1011 with CRC)
  return { command: 'NAK', params: {}, crcValid: null };
}

function parseReqRN(bits: number[]): ParsedCommand {
  // 11000000 + RN16(16) + CRC16(16) = 40 bits
  if (bits.length < 40)
    return { command: 'ReqRN', params: { partial: true }, crcValid: null };
  ...
  return {
    command: 'ReqRN',
    params: { rn16 },
    crcValid: crcVal === crcExpected,
  };
}

function parseRead(bits: number[]): ParsedCommand {
  // 11000001 + MemBank(2) + WordPtr(EBV) + WordCount(8) + RN(16) + CRC16(16)
  if (bits.length < 26)
    return { command: 'Read', params: { partial: true }, crcValid: null };
...
function parseWrite(bits: number[]): ParsedCommand {
  // 11000010 + MemBank(2) + WordPtr(EBV) + Data(16) + RN(16) + CRC16(16)
  if (bits.length < 26)
    return { command: 'Write', params: { partial: true }, crcValid: null };
  const memBank = MEMBANK_MAP[bitsToInt(bits, 8, 2)];
  const ptrResult = extractEBV(bits, 10);
  const wordPtr = ptrResult.value;
  let pos = 10 + ptrResult.bitsConsumed;
...
function parseBlockErase(bits: number[]): ParsedCommand {
  // 11000111 + MemBank(2) + WordPtr(EBV) + WordCount(8) + RN(16) + CRC16(16)
  if (bits.length < 26)
    return { command: 'BlockErase', params: { partial: true }, crcValid: null };
  const memBank = MEMBANK_MAP[bitsToInt(bits, 8, 2)];
  const ptrResult = extractEBV(bits, 10);
  const wordPtr = ptrResult.value;
  let pos = 10 + ptrResult.bitsConsumed;
...
function parseAuthenticate(bits: number[]): ParsedCommand {
  ...
  const csiStr =
    csi === 0 ? 'Crypto Suite 0' : csi === 1 ? 'AES-128' : `CSI(${csi})`;
  return {
    command: 'Authenticate',
    params: {
      CSI: csiStr,
      rawBits: bitsToHex(bits, 10, Math.min(bits.length - 10, 128)),
    },
    crcValid: null,
  };
}

function parseAuthComm(bits: number[]): ParsedCommand {
  // 11010001 + variable
  return {
    command: 'AuthComm',
    params: {
      rawBits:
        bits.length > 8
          ? bitsToHex(bits, 8, Math.min(bits.length - 8, 128))
          : '',
    },
    crcValid: null,
  };
...
function parseSecureComm(bits: number[]): ParsedCommand {
  // 11010010 + variable
  return {
    command: 'SecureComm',
    params: {
...
function parseTagPrivilege(bits: number[]): ParsedCommand {
  // 11010100 + variable
  return {
...
function parseQueryX(bits: number[]): ParsedCommand {
  // 11100000 + variable
  return {
    command: 'QueryX',
    params: {
...
function parseQueryY(bits: number[]): ParsedCommand {
  // 11100001 + variable
  return {
    command: 'QueryY',
    params: {
...
export class PIEDecoder {
  sampleRate: number;
  centerFreqMHz: number;
  onMessage: ((msg: PIEMessage) => void) | null;

  // ---- Lowpass filter ----
  private _lpAlpha: number;
  private _lpState: number;

  // ---- Adaptive threshold ----
  private _emaHigh: number;
  private _emaLow: number;
  private _emaAlpha: number;
  private _threshold: number;
  private _thresholdInitialized: boolean;

  // ---- Edge detection ----
  private _prevBinary: number;
  private _globalSampleOffset: number;
  private _lastEdgeOffset: number;

  // ---- PIE state machine ----
  private _state: number;
  private _tari: number;
  private _rtcal: number;
  private _pivot: number;
  private _bits: number[];
  private _lastHighDuration: number;
  private _lastLowDuration: number;
  private _symbolStart: number;
  private _cwTimeout: number;

  // ---- Timing limits in us ----
  private _delimiterMinUs: number;
  private _delimiterMaxUs: number;
  private _tariMinUs: number;
  private _tariMaxUs: number;

  // ---- Round tracking ----
  private _roundId: number;
  private _pendingSelectRoundId: number | null;

  // ---- Carry-over buffer for filter continuity ----
  private _carryOver: Float32Array | null;
  private _carryOverLen: number;

  // ---- Message ID counter ----
  private _nextId: number;

  // ---- Statistics ----
  stats: PIEDecoderStats;

  constructor({
    sampleRate = 2400000,
    centerFreqMHz = 915,
    onMessage = (() => {}) as (msg: PIEMessage) => void,
  }: Partial<PIEDecoderOptions> = {}) {
    ...
    this._cwTimeout = 0; // CW gap timeout in samples (3 x RTcal)
   ...
   */
  process(iqFloat32Array: Float32Array): void {
  ...
  private _updateThreshold(filtered: Float32Array): void {
  ...
  private _samplesToUs(samples: number): number {
    return (samples / this.sampleRate) * 1e6;
  }

  private _onFallingEdge(pos: number, _highDuration: number): void {
    switch (this._state) {
      case PIE_STATE.IDLE:
      ...
      case PIE_STATE.WAIT_RTCAL: {
        ...
        if (this._tari === 0) {
          ...
          // RTcal should be > 2.5 x Tari and < 3 x 4 x Tari (generous range)
  ...
  private _onRisingEdge(pos: number, lowDuration: number): void {
  ...
  private _emitMessage(parsed: ParsedCommand): void {
    ...
    const roundId =
      isSelect && this._pendingSelectRoundId !== null
        ? this._pendingSelectRoundId
        : this._roundId;

    const msg: PIEMessage = {
      id: this._nextId++,
      direction: 'R2T',
